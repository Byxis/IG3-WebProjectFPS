...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    currentErrorType === ErrorTypes.SERVER_UNREACHABLE && autoRetry &&
    countdownElement
  ) {
  ...
  function startReconnectCountdown() {
    ...
    const interval = setInterval(() => {
      ...
      if (countdown <= 0) {
        ...
        if (
          currentErrorType === ErrorTypes.AUTH_REQUIRED ||
          ...
          refreshAuthToken()
            .then((refreshed) => {
              if (refreshed) {
                ...
                globalThis.location.href = `login?error=${currentErrorType}`;
              }
            })
            .catch(() => {
              globalThis.location.href = `login?error=${currentErrorType}`;
        ...
        checkServerConnection()
          .then((result) => {
            if (result.isConnected) {
              ...
              if (
                currentErrorType === ErrorTypes.ACCESS_DENIED ||
            ...
            if (reconnectAttempts <= maxReconnectAttempts) {
              ...
              errorMessage.textContent =
                `Unable to connect to the game server after ${reconnectAttempts} attempts.`;

              autoRetry.innerHTML =
                'Automatically retrying in <span id="countdown">5</span> seconds...';
              countdownElement = document.getElementById("countdown");
              startReconnectCountdown();
            } else {
              autoRetry.textContent =
                "Maximum reconnection attempts reached. Please try manually.";
            }
          });
      }
    }, 1000);
  ...
  retryButton.addEventListener("click", async () => {
    if (
      ...
      retryButton.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> Trying...';
...
function configureErrorDisplay(type, attempts, redirectFrom) {
  ...
  switch (type) {
    case ErrorTypes.SERVER_UNREACHABLE:
      ...
      errorMessage.textContent =
        `Unable to connect to the game server after ${attempts} attempts.`;
      errorDetails.textContent =
        "The server may be down for maintenance or experiencing high load. Please try again later.";
    ...
    case ErrorTypes.AUTH_FAILED:
      ...
      errorDetails.textContent =
        "Please log in again or contact support if this persists.";
      retryButton.style.display = "none";
      autoRetry.style.display = "none";
      loginButton.style.display = "block";
      break;
    ...
    case ErrorTypes.BANNED:
      ...
      errorMessage.textContent =
        "Your account has been suspended from this server.";
      errorDetails.textContent =
