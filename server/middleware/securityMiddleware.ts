...
export function rateLimiter(ctx: Context, next: () => Promise<unknown>) {
    ...
    if (loginAttempts.has(ip)) {
        ...
        if (attempts.count >= 5 && now - attempts.timestamp < 15 * 60 * 1000) {
            ...
            ctx.response.status = ErrorTypes.TOO_MANY_REQUESTS;
            ctx.response.body = { error: "Too many login attempts. Please try again later." };
            return;
        }
        ...
        if (now - attempts.timestamp > 15 * 60 * 1000) {
            ...
            loginAttempts.set(ip, { count: attempts.count + 1, timestamp: attempts.timestamp });
        }
    } else {
        loginAttempts.set(ip, { count: 1, timestamp: now });
    }
