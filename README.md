...
Jeu de tir à la première personne multijoueur en temps réel développé avec une architecture client-serveur sécurisée. Authentification robuste, validation anti-triche côté serveur et système de modération avancé.
...
- **Client** (Frontend) : Three.js pour le rendu 3D, WebSockets pour la communication
- **Server** (Backend) : Deno/Oak avec validation côté serveur
- **Shared** : Physique et configuration hébergé côté serveur puis envoyé au client
...
**Production :** Le jeu est accessible sur [webwarfare.cluster-ig3.igpolytech.fr](https://webwarfare.cluster-ig3.igpolytech.fr)
...
**Formats de durée** : `5m` (minutes), `2h` (heures), `1d` (jours), `1w` (semaines)
...
### Sons d'Interface
- **Munitions vides** (`empty.mp3`, `dry-fire.mp3`, `dry-fire-high.mp3`) : Feedback d'arme déchargée
- **Audio spatialisé** pour l'immersion 3D
- **Synchronisation** avec les animations visuelles

### Sources Audio et Copyright
- **Sons gratuits** téléchargés depuis des sites comme Voicy et autres plateformes libres
- **Usage éducatif** dans le cadre d'un projet d'apprentissage
- **Politique copyright** : Si un son est protégé par des droits d'auteur, merci de me contacter pour suppression immédiate
...
- **VS Code Tasks** pour le développement local
  - `Start Frontend` : Démarre le serveur front et se relance en cas de modification
  - `Start Backend` : Démarre le serveur back et se relance en cas de modification  
...
- **Rendu 3D avancé** : Moteur Three.js avec raycasting pour détection de collision précise → [Architecture](#️-architecture)
- **Anti-triche robuste** : Validation côté serveur de tous les mouvements et actions → [Sécurité](#-sécurité)
- **Sound Design immersif** : Système audio spatialisé avec feedback tactile complet → [Sound Design](#-sound-design)
- **Architecture temps réel** : Synchronisation WebSocket optimisée avec compensation de latence → [Technologies](#-technologies)
- **Hot-reload développement** : Outils VS Code automatisés pour productivité maximale → [CI/CD](#️-cicd-et-outils-de-développement)
- **Effet Parallax moderne** : Interface responsive avec animations géométriques fluides → [Interface et Effets](#-interface-et-effets)
- **Gestion d'erreurs intelligente** : Reconnexion automatique avec backoff exponentiel → [Gestion des Erreurs](#-gestion-des-erreurs)
- **Physique partagée** : Moteur physique synchronisé client/serveur pour cohérence → [Architecture](#️-architecture)
- **Rate limiting** : Système de limitation de requêtes sur login/register contre attaques brute-force → [Sécurité](#-sécurité)
...
- **Optimisation et débogage** : Suggestions d'améliorations et corrections de bugs
...
- **Maintien de la qualité** : L'IA complète les compétences sans remplacer la réflexion

> 💡 **Note importante** : L'utilisation de l'IA a servi d'assistance au développement tout en préservant l'authenticité du travail académique et l'acquisition de compétences techniques.
...
**WebWarfare** - Projet éducatif développé dans le cadre des études à **Polytech Montpellier** en **Informatique et Gestion (IG3)** pour le cours d'**Architecture Web**. 

Objectif pédagogique : Maîtrise des architectures client-serveur modernes, sécurité web, et développement temps réel avec technologies natives.
